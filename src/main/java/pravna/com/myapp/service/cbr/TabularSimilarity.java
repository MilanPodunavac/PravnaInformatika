...
public class TabularSimilarity implements LocalSimilarityFunction {
    ...
    public TabularSimilarity(List<String> categories) {
        ...
        for (int i = 0; i < n; i++) matrix[i][i] = 1; // symbolic similarity of term with itself
    }
    ...
    public double compute(Object value1, Object value2) throws NoApplicableSimilarityFunctionException {
        if (value1 instanceof String && value2 instanceof String) return compute((String) value1, (String) value2);
        if (value1 instanceof List && value2 instanceof List) return compute((List) value1, (List) value2);
        return 0;
    }
    ...
    public double compute(String str1, String str2) {
        ...
        if (str1 != null && str1.equals(str2)) return 1;
        return 0;
    }
    ...
    public double compute(List<String> list1, List<String> list2) {
        ...
        for (String el1 : list1) {
            double sim = 0;
            for (String el2 : list2) sim = Math.max(sim, compute(el1, el2));
            sim1to2 += sim;
        }
        double sim2to1 = 0;
        for (String el2 : list2) {
            double sim = 0;
            for (String el1 : list1) sim = Math.max(sim, compute(el2, el1));
            sim2to1 += sim;
        }
    ...
    public boolean isApplicable(Object value1, Object value2) {
        if (value1 instanceof String && value2 instanceof String) return true;
        if (value1 instanceof List && value2 instanceof List) return true;
        return false;
    }
}
